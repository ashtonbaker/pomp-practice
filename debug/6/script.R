...
rmultinom(L_0, L_rate, LSTAGES, L);
rmultinom(P_0, P_rate, PSTAGES, P);
A = 100;")
...
double gamma_E = ESTAGES / (tau_E + ESTAGES) * exp((-cel * L_tot - cea * A) / ESTAGES);
double gamma_L = LSTAGES / (tau_L + LSTAGES) * (1 - mu_L);
double gamma_P = (PSTAGES / tau_P) * exp((-cpa * A) / ESTAGES);

double mu_e = ESTAGES / (tau_E + ESTAGES) * (1 - exp((-cel * L_tot - cea * A) / ESTAGES));
double mu_l = (LSTAGES / tau_L) * mu_L;
...
for (k = 0; k < ESTAGES; k++) {
  ...
  etrans[2*k+1] = rbinom(E[k]-etrans[2*k],mu_e);
}

for (k = 0; k < LSTAGES; k++) {
rate[0] = gamma_L;
rate[1] = mu_l;
reulermultinom(2,L[k],&rate[0],dt,&ltrans[2*k]);
}

for (k = 0; k < PSTAGES; k++) {
rate[0] = gamma_P;
rate[1] = mu_p;
reulermultinom(2,P[k],&rate[0],dt,&ptrans[2*k]);
}

reulermultinom(1,A,&mu_A,dt,&adeath);
...
if (A_obs > 0) {
  lik += log(pnorm(A_obs + 0.5, A_tot, meas_sd, 1, 0) - pnorm(A_obs - 0.5, A_tot, meas_sd, 1, 0));
} else {
  lik += pnorm(0.5,A_tot,meas_sd,1,1);
}
