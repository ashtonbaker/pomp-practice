...
panelPomp(
  object = pompList,
  shared = coef(model)[c("b", "cea", "cel", "mu_A", "mu_L", "tau_E", "tau_L", "tau_P", "od")] -> shared.params,
  specific = matrix(
    data = coef(model)[c("cpa")] -> specific.params,
...
stew(file="./output/box_search_global.rda",{
  ...
  t_global <- system.time({
    ...
    {
      mf <- mif2(mf1,start=c(shared=unlist(guess)[c('cpa')], specific=()),tol=1e-60)
      mf <- mif2(mf,Nmif=100)
